...
The import graph becomes flat (containers import containers and their own view; views import nothing) while the render tree stays as deep as it needs to be. React puts the two together at runtime.
...
packages/
  eslint-plugin/               eslint-plugin-superarchitecture: 12 rules + standalone runner
  stylelint-config/            stylelint-config-superarchitecture: BEM + tokens rules
examples/
  board/                       a kanban board written by the rules (containers, views, hooks, slice)
```
...
**Read.** Start with `docs/02-containers-and-views.md` (the rule) and `docs/03-data-flow.md` (how data reaches views). Then `01` for where files go, `04` for modules, `05` for CSS. Each document ends with a checklist.
...
**Look at the example.** `examples/board/` is small enough to read in ten minutes: eight containers, their views, a slice with small writes, and a `hooks.ts` that is the whole data API.
...
- Talk: *Jedno jednoduché pravidlo, které změnilo, jak píšu React aplikace* (Frontendisti, September 2026). The slides are in the talk's own folder, not in this repo.
- Talk: the CSS part comes from an earlier Frontendisti talk on BEM and custom properties (June 2026).
- Analyzer and viewer used to measure the examples: the `twographs` repository.
